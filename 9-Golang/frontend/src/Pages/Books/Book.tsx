...
const Book = () => {
    ...
    React.useEffect(() => {
        async function fetchBook(url: string) {
            ...
            if (response.ok) {
                ...
                setBook(book);
                form.setFieldsValue({ id: book.id });
                ...
                setIsbnRaw(book.isbn);
                form.setFieldsValue({ publication: moment(book.publication) });
                let imgUrl: string = await getCover(book.cover);
                form.setFieldsValue({ cover: [{ uid: '-1', name: book.cover, status: 'done', thumbUrl: imgUrl }] });
            }
            else
                console.error(error);
        }
    ...
    async function getCover(fileName: string) {
        ...
        const response = await fetch(`https://localhost:5002/api/book/getImage?filename=${fileName}`, {
            method: "GET",
            headers: {
