...
const BookCreate = () => {
    ...
    const onFinish = (values: any) => {
        ...
        confirmMessage(
            'save',
            async function(){
                const { response, error, json } = await request("https://localhost:5002/api/Book", "CREATE", values)
                response.ok ? values.cover ? saveImage(json.id, file) : resultMessage('save') : resultMessage('save', error)
            }
        )
    }
